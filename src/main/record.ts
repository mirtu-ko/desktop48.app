...
ipcMain.handle('recordTaskStart', async (event: IpcMainInvokeEvent, url: string, filename: string, liveId: string) => {
  ...
  const startPromise = new Promise<string>((resolve, reject) => {
    ...
    ffmpeg.once('error', (error) => {
      const errMsg = `[record.ts]ffmpeg failed to start: ${error.message}`
      warn(errMsg)
  ...
  // IPC 调用只表示 ffmpeg 已成功启动；录制结束由 recordTaskEnd/recordTaskError 通知。
  return startPromise
})
